...
__device__ __forceinline__ void argmax_combine(VT& v, int& i, VT ov, int oi) {
    ...
    // returned an out-of-vocab index (observed as token id == vocab_size,
    // which poisoned last_sampled_tokens/input_ids downstream). NaN rows now
    // degrade like all--inf rows instead.
